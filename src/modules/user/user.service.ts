...
export class UserService {
  ...
  // PUT
  async replace(id: string, input: { email: string; password: string; role: UserRole }): Promise<UserEntity | null> {
    const existing = await this.userDb.findById(id);
    if (!existing) throw new ApiError(404, { message: "User not found" });

    const email = input.email.trim().toLowerCase();
    if (!email.includes("@")) throw new ApiError(400, { message: "Email must include @" });
    const dup = await this.userDb.findByEmail(email);
    if (dup && dup._id.toHexString() !== id) throw new ApiError(409, { message: "Email already exists" });

    const pwd = input.password.trim();
    if (pwd.length < 6) throw new ApiError(400, { message: "Password must be at least 6 characters" });
    if (!/[A-Z]/.test(pwd)) throw new ApiError(400, { message: "Password must contain at least 1 uppercase letter" });
    if (!/[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(pwd)) throw new ApiError(400, { message: "Password must contain at least 1 special character" });
  ...
  // PATCH
  async update(id: string, input: Partial<{ email: string; password: string; role: UserRole }>): Promise<UserEntity | null> {
    ...
    const user = await this.userDb.updateOne(id, update as any);
    if (!user) throw new ApiError(404, { message: "User not found" });
    return user;
  }
